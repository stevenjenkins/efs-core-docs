...
simplest, minimal set of software when instaling the OS, and then let
the efs_virtual_machine script handle the rest.
...
The first VM must be an x86_64 CentOS 5.4 server.  The other test
machine pairs can be any supported EFS client operating .  However,
the code to setup the core services has only been tested on CentOS
5.4.
...
then reboot the machine when successful.
...
requirement for CentOS 5.4 holds for boot.efs as well.

Copy efs_virtual_machine to the new VM and run:

    efs_virtual_machine d.bar.baz.reg.boot.efs core

This will do the same thing is does for the test.efs server.
Configure core services and reboot.

...
In the shell you're using to run "prove ... test/*", invoke perl as
follows:
...
In the shell you're using to run "perl Makefile.PL && make", you will
run the efs client command, but first you have to deal with
authentication.  When the efs client is run in the test suite, code in
EFS/Test.pm takes care of acquiring Kerberos credentials, but you will
need to do this manually as follows:

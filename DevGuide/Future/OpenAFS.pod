...
continued to upgrade this module, and we beleive they are on version
1.11.
...
we're testing EFS funcionality.
...
administrative domain.  As discussed near the beginning of this
document, it is entirely unclear how we will need to map this
administrative domain to the underlying technologies like Kerberos,
DNS, etc.
...
We make essentially the same assumption with NFS, as well.  It is an
impliciit assumption, that we can neither audit, nor enforce, that
administrators of the EFS NFS servers are only exporting these
filesystems to clients that are part of a centrally managed uid/gid
namespace, and that is proving to be a very difficult requirement for
most large sites to satisfy.

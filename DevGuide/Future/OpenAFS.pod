...
also suffers from similar limitations, and in most real worl
...
challenge was on the securtity side, and since we were building our
own Kerberos infrastructrue, we were able to address these issues, but
...
that noone adopts it), both /efs/dev and /efs/dist can be simple
...
additonal volumes which make up the namespace will be mounted.  We
...
This makese it very difficult to represent the pathnames where each
...
shorter the index, and we want to leabve plenty of room for the index
to grow.  Over 15 years of using AFS/VMS with a similar architecture,
the volume index has grown to.... (XXX: ask someone at MS -- I think
...
=head2 Volume Mount Point Heirarchy

The namespace can then be implemented in a volume heirarchy as follows.
...
In the NFS-based EFS cells, the coupling bewteen EFS and NFS is very
...
We will need a new colume in the "cell" table to track the filesystem
type of the cell.  I am proposing that each distinct cell be defined
as either "nfs" or "afs", since the differences between NFSv3 and
NFSv4 are relatively small, I think v3 vs v4 can be managed as
...
then fine -- not worth fighting over, but I like short and sweet
keywords).

=head2 Fileservers and Fileshares

Long term, I want to take over the world, and extend EFS to support
...
clone of the readwrite volume, something that I would hope in 2010 is
...
In the EFS NFS implementation, we solved the analgous problem with the
...
mind that individual EFS servers can support an abitrary number of
...
address the use of this object in the NFS cases, I think we can simply
...
I was the original author of AFS::Command, and as far as CPAN is
concerned, I still own it.  The last version I uploaded to CPAN before
quitting Morgan Stanley was 1.7, however, the engineers at MS have
continued to upgrade this module, and I beleive they are on version
1.11.

I've warned them that I want my code back, and I will work with them
to get the latest release posted on CPAN, and I only mention this here
because this could prove to be a problem for them.  Rumor has it that
the version they are using is NOT generic enough to be published on
CPAN, in which case, they will be forced to maintain that as a forked
version, or clean it up.  

I will also use the other AFS::* perl modules, where appropriate, and
I am looking forward to improving how those modules interoperate, and
...
the distribution mechanism in AFS/VMS, I want to avoid that approach.

...
Note that this script also does not need to be run as root.  It can be
run as a non-root user, provided it has write access to
/var/efs/etc/passwd and /etc/efs.
...
The next stage of the bootstrapping process involves building the EFS
distribution on each of the UNIX platforms you have chosen to support.

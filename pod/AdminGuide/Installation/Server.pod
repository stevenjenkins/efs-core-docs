...
As root:
    ...
    su efsops -c ./util/efs_bootstrap_namespace
...
run all 4 of the scripts above with one simple wrapper:
...
We require either a MySQL database, with three database logins for the
EFS application.

...
efs_platform script, and rc.efs, as well as defined properly in the
EFS database.
...
team, rarely worth the effort for the majoriry of the software managed
...
A platform is always defined with one or more specific instance
...
There are two different ways that platform instances are used, one
critical to the implemenation of the EFS client, and one optional
...
sparc32.sunos.10.  The directory tree in /efs/dist will have the
following pair of directories.

    /efs/dist/foo/bar/1.0/.exec/x86-32.rhel.5
    /efs/dist/foo/bar/1.0/.exec/sparc32.sunos.10

If you have defined the default EFS platforms and instances, then EFS
will also create a set of symlinks in the .exec directory as follows.

For each of the backwards compatible platforms:

    /efs/dist/foo/bar/1.0/.exec/x86-32.rhel.6 -> x86-32.rhel.5
    /efs/dist/foo/bar/1.0/.exec/sparc32.sunos.11 -> sparc32.sunos.10

For each of the emulating platforms (used when a 64bit platform can
emulate the instruction set of it's 32 bit version):
    ...
    /efs/dist/foo/bar/1.0/.exec/sparc64.sunos.10 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sparc64.sunos.11 -> sparc32.sunos.10
...
namespace.  In addition, EFS will create symlinks for each instance to
it's underlying platform.  This will include all of the following:
    ...
    /efs/dist/foo/bar/1.0/.exec/sun4u32.sunos.10 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4u32.sunos.10 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4u64.sunos.10 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4u64.sunos.10 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4u32.sunos.11 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4u32.sunos.11 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4u64.sunos.11 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4u64.sunos.11 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4v32.sunos.10 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4v32.sunos.10 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4v64.sunos.10 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4v64.sunos.10 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4v32.sunos.11 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4v32.sunos.11 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4v64.sunos.11 -> sparc32.sunos.10
    /efs/dist/foo/bar/1.0/.exec/sun4v64.sunos.11 -> sparc32.sunos.10

This is great, if you are accessing this software using a platform of
...
mount for /efs/dist, the /.efs/dist mount is instance specific.  For
example, the mounts typically look something like:
...
Note that /.efs/dist is a mount point for a instance specific
subdirectory of the /efs/dist volume.  On this particular client, that
second symlink is:

    /.efs/dist/foo/bar/1.0/exec -> /efs/dist/foo/bar/1.0/.exec/amd64.rhel.5

Since /.efs/dist is instance specific, this second symlink is always
instance specific, and this indirection accomplishes for NFS (v3 and
v4) what the AFS client does as follows:
...
and install or dist a release while the namespace is in flux.

=head2 Defining the Initial Platform Namespace

When a new EFS domain is bootstrapped, the standard default set of EFS
platforms and instances for the supported UNIX-like platforms are all
defined for you.  This default list will be extended as the EFS
community grows, and the product is ported to more platforms.
Defining the initial platform and instance namespace can be done by
altering these configuration files in the source tree:
...
Anyone wishing to extend this namespace to an new, as yet unsupported
...
However, it is worth pointing out than in the legacy EFS 2 environment, 
...
operating systems.  For example, RHEL 6 doens't exist yet, but we know
...
'efs execlinks any' for how this operations can be controlled.  The
...
want to forget about Solaris 10.  The Solaris 11 platforms would be:
    ...
    Platform         Backwards  Emulates
    ========         =========  ========
    x86-32.sunos.11  NULL       NULL
    x86-64.sunos.11  NULL       x86-32.sunos.11

The instances would be:

    PlatformInstance  Platform         Backwards  Emulates
    ================  ========         =========  ========
    amd32.sunos.11    x86-32.sunos.11  NULL       NULL
    amd64.sunos.11    x86-64.sunos.11  NULL       amd32.sunos.11
    ia32.sunos.11     x86-32.sunos.11  NULL       NULL
    ia64.sunos.11     x86-64.sunos.11  NULL       ia32.sunos.11
...
First, define the new root platforms and instances:
    ...
    efs define platform x86-32.sunos.10
    efs define platform x86-64.sunos.10 --emulates x86-32.sunos.10

    efs define platforminstance amd32.sunos.10 x86-32.sunos.10
    efs define platforminstance amd64.sunos.10 x86-64.sunos.10 \
        --emulates amd32.sunos.10
    efs define platforminstance ia32.sunos.10 x86-32.sunos.10
    efs define platforminstance ia64.sunos.10 x86-64.sunos.10 \
        --emulates ia32.sunos.10

Next, redefine the Solaris 11 names to refer to the new Solaris 10
names.

    efs define platform x86-32.sunos.11 x86-32.sunos.10
    efs define platform x86-64.sunos.11 x86-64.sunos.10 x86-32.sunos.11

    efs define platforminstance amd32.sunos.11 x86-32.sunos.11 amd32.sunos.10
    efs define platforminstance amd64.sunos.11 x86-64.sunos.11 \
        amd64.sunos.10 amd32.sunos.11
    efs define platforminstance ia32.sunos.11 x86-32.sunos.11 ia32.sunos.10
    efs define platforminstance ia64.sunos.11 x86-64.sunos.11 \
        ia64.sunos.10 ia32.sunos.11

The new platform and instance definitions now look like this:

    Platform         Backwards        Emulates
    ========         =========        ========
    x86-32.sunos.10  NULL             NULL
    x86-32.sunos.11  x86-32.sunos.10  NULL
    x86-64.sunos.10  NULL             x86-32.sunos.10
    x86-64.sunos.11  x86-64.sunos.10  x86-32.sunos.11

    PlatformInstance  Platform         Backwards       Emulates
    ================  ========         =========       ========
    amd32.sunos.10    x86-32.sunos.10  NULL            NULL
    amd32.sunos.11    x86-32.sunos.11  amd32.sunos.10  NULL
    amd64.sunos.10    x86-64.sunos.10  NULL            amd32.sunos.10
    amd64.sunos.11    x86-64.sunos.11  amd64.sunos.10  amd32.sunos.11
    ia32.sunos.10     x86-32.sunos.10  NULL            NULL
    ia32.sunos.11     x86-32.sunos.11  ia32.sunos.10   NULL
    ia64.sunos.10     x86-64.sunos.10  NULL            ia32.sunos.10
    ia64.sunos.11     x86-64.sunos.11  ia64.sunos.10   ia32.sunos.11
...
and 64 in B<all> of the chipset names.  In addition, we decided to
drop "5." from the osrel tokens, and just use the Solaris release
number (10, 11, etc).
...
Next, we rename the platforms and the instances:
    ...
    efs rename platform sparc.sunos.5.10 sparc32.sunos.10
    efs rename platform sparc.sunos.5.11 sparc32.sunos.11

    efs rename platforminstance sun4u.sunos.5.10 sun4u32.sunos.10
    efs rename platforminstance sun4u.sunos.5.11 sun4u32.sunos.11
    efs rename platforminstance sun4v.sunos.5.10 sun4v32.sunos.10
    efs rename platforminstance sun4v.sunos.5.11 sun4v32.sunos.11

The platform and instance namespace now looks like:

    Platform           Backwards          Emulates
    ========           =========          ========
    sparc32.sunos.10   NULL               NULL
    sparc32.sunos.11   sparc32.sunos.10   NULL

    PlatformInstance    Platform           Backwards         Emulates
    ================    ========           =========         ========
    sun4u32.sunos.10    sparc32.sunos.10   NULL              NULL
    sun4u32.sunos.11    sparc32.sunos.11   sun4u32.sunos.10  NULL
    sun4v32.sunos.10    sparc32.sunos.10   NULL              NULL
    sun4v32.sunos.11    sparc32.sunos.11   sun4v32.sunos.11  NULL
...
created for the old names.
    ...
    Alias              Platform
    =====              ========
    sparc.sunos.5.10   sparc32.sunos.10
    sparc.sunos.5.11   sparc32.sunos.11
    
    Alias              PlatformInstance
    ================   ================
    sun4u.sunos.5.10   sun4u32.sunos.10
    sun4u.sunos.5.11   sun4u32.sunos.11
    sun4v.sunos.5.10   sun4v32.sunos.10
    sun4v.sunos.5.11   sun4v32.sunos.11
...
for new instals in new releases, going forward.  You may very well

...
which have been from the source rpm.  The specific example is gcc and
binutils.  We provide the release of gcc from gnu.org in the gnu/gcc
project, but RedHat has been patching gcc and providing releases with
important bug fixes which are often not found in the gnu.org releases
right away.  Therefore, we provide EFS-integrated builds of the RHEL
gcc packages in the rhel/gcc project, independent of the release found
in gnu/gcc.
...
Presumably, we the need arises, we will also create metaprojs with
...
not need to be involved in the normal user of the metaproj for
